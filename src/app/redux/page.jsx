import Link from "next/link";
import { reduxDays } from "@/app/redux/data";
import { Callout } from "@/components/notes/Callout";

export const metadata = { title: "Redux Notes" };

export default function ReduxHome() {
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-semibold tracking-tight text-[#764abc]">
          Redux
        </h1>
        <Callout variant="reduxIntro">
          <p className="text-sm text-[#2c3e50]">
            Learn the “why” of Redux by feeling the pain of prop drilling, then
            follow the natural evolution to Redux Toolkit.
          </p>
        </Callout>
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        {reduxDays.map((day) => (
          <Link
            key={day.slug}
            href={`/redux/${day.slug}`}
            className="rounded-xl border bg-white p-5 text-[#195568] shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
          >
            <div className="text-lg font-semibold tracking-tight text-[#764abc]">
              {day.title}
            </div>
            <div className="mt-1 text-sm text-muted-foreground">
              {day.description}
            </div>
            <div className="mt-3 text-sm text-muted-foreground">
              {day.lessons.length} lesson{day.lessons.length === 1 ? "" : "s"} →
            </div>
          </Link>
        ))}
      </div>
    </div>
  );
}

