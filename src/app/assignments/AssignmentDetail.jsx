...
const markdownComponents = {
  h1: (props) => <h1 className="mt-8 text-2xl font-semibold text-[#0174af] first:mt-0" {...props} />,
  h2: (props) => <h2 className="mt-8 text-xl font-semibold text-[#195568]" {...props} />,
  h3: (props) => <h3 className="mt-6 text-base font-semibold text-[#195568]" {...props} />,
  ...
  code: (props) => <code className="rounded bg-[#e6f5fb] px-1.5 py-0.5 text-[0.85em] text-[#195568]" {...props} />,
  pre: (props) => <pre className="mt-3 overflow-x-auto rounded-lg bg-[#0b1220] p-4 text-sm text-white [&_code]:bg-transparent [&_code]:p-0 [&_code]:text-inherit" {...props} />,
...
export default function AssignmentDetail({ assignment, area, technology, topic }) {
  ...
  return <div className="mx-auto flex max-w-3xl flex-col gap-8">
    <nav className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground" aria-label="Breadcrumb">
      <Link href="/assignments" className="hover:text-foreground">Assignments</Link><span>/</span>
      <Link href={`/assignments/${area.slug}/${technology.slug}`} className="hover:text-foreground">{technology.title}</Link><span>/</span>
      <Link href={topicPath} className="hover:text-foreground">{topic.title}</Link>
    </nav>
    <div className="flex flex-col gap-4 rounded-xl border bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      <div className="flex gap-4"><div className="flex size-11 shrink-0 items-center justify-center rounded-lg bg-[#e6f5fb] text-sm font-bold text-[#0174af]">{assignment.number}</div><div>
        <h1 className="text-2xl font-semibold tracking-tight text-[#195568]">{assignment.title}</h1>
